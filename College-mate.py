...
app = Flask(__name__)
app.secret_key = "abc"
# Dynamic upload directories based on app root
BASE_DIR = os.path.abspath(os.path.dirname(__file__))
path1 = os.path.join(BASE_DIR, 'static', 'student_photos') + os.sep
path = os.path.join(BASE_DIR, 'static', 'Staff') + os.sep
path2 = os.path.join(BASE_DIR, 'static', 'syllabus') + os.sep
path3 = os.path.join(BASE_DIR, 'static', 'notes') + os.sep
path4 = os.path.join(BASE_DIR, 'static', 'qp') + os.sep
...
def add_staff():
    if session['lg'] == 'lin':
        if request.method == "POST":
            ...
            staff_photo = "/static/Staff/" + d + '.jpg'
            db = Db()
...
def view_student():
    if session['lg'] == 'lin':
        ...
        print(res)
        return render_template('staff/view_students.html', data=res)
    else:
        return redirect('/')
...
def delete_staff_allocation(aid):
    if session['lg'] == 'lin':
        ...
        db = Db()
        db.delete("delete from subject_alloc WHERE suballoccourseid ='" + aid + "'")
        return redirect('/course_allocation_view')
    else:
        return redirect('/')
...
#     re('https://meet.google.com/landing')
...
def Exam_schedule():
    if session['lg'] == "lin":
        if request.method == "POST":
            ...
            if q is None:
                db.insert("insert into exam (date, time, subject_alloc_id, marks) VALUES ('" + date + "','" + time + "','" + subject + "','" + m + "')")
                    ...
                    if mrk == 0:
                        ...
                        if qq is None:
                            if t == 'MCQ':
                                ...
                                db = Db()
                                db.insert(
                                    "insert into question (exam_id, question, option1, option2, option3, option4, correct, marks, exam_type) VALUES ('" + eid + "','" + question + "','" + option1 + "','" + option2 + "','" + option3 + "','" + option4 + "','" + correct + "', '" + marks + "','" + t + "')")
                                return '''<script>alert('Added');window.location="/add_QandA/''' + eid + '''/''' + m + '''"</script>'''
                            else:
                                db = Db()
                                db.insert(
                                    "insert into question (exam_id, question, option1, option2, option3, option4, correct, marks, exam_type) VALUES ('" + eid + "','" + question + "','','','','','" + correct + "', '" + marks + "','" + t + "')")
                                return '''<script>alert('Added');window.location="/add_QandA/''' + eid + '''/''' + m + '''"</script>'''
                        else:
